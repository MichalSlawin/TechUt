...
public class DoorManagerJDBC implements DoorManager {
    ...
    private String createTableDoor = "CREATE TABLE Door(id bigint GENERATED BY DEFAULT AS IDENTITY, " +
            "producer varchar(20), productionDate Date, isExterior bit, weight double, CHECK(weight > 0.0))";
    ...
    public DoorManagerJDBC() {
        try {
            ...
            findDoorByIdStmt = connection.prepareStatement("SELECT * FROM Door WHERE id=?");
            findDoorByProducerStmt = connection.prepareStatement("SELECT * FROM Door WHERE producer=?");
    ...
    public List<Door> getAllDoors() {
        return getDoors(getAllDoorsStmt);
    }

    @Override
    public List<Door> findDoorById(long id) {
        try {
            findDoorByIdStmt.setBigDecimal(1, new BigDecimal(id));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return getDoors(findDoorByIdStmt);
    }

    @Override
    public List<Door> findDoorByProducer(String producer) {
        try {
            findDoorByProducerStmt.setString(1, producer);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return getDoors(findDoorByProducerStmt);
    }
